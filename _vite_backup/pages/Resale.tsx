    ...
    <div className="luxe-landing min-h-screen bg-background-light dark:bg-background-dark text-primary font-display">
      <div className="max-w-7xl mx-auto px-6 mt-8">
        <div className="flex gap-10 min-h-[calc(100vh-64px-4rem)]">
          {/* Sidebar Filters - 320px fixed, sticky, visible on lg+ */}
          <aside className="w-80 shrink-0 hidden lg:block">
            <div className="sticky top-24 h-[calc(100vh-7rem)] overflow-y-auto bg-white dark:bg-background-dark/50 rounded-xl shadow-sm border border-primary/10 p-6 space-y-8">
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-32">
          {resales.map((p) => {
            ...
            const badges = 'status' in p && p.status ? [String(p.status)] : []
            return (
            <Link
              key={p.id}
              to={`/property/${p.id}`}
              ...
              <div className="relative aspect-[4/3] overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex flex-col gap-2">
                  {badges.map((badge) => (
                    <span
                      ...
                      className={
                        badge === 'Featured'
                          ? 'px-3 py-1 bg-accent-gold text-white text-[10px] font-bold uppercase tracking-widest rounded-full shadow-lg'
                          : 'px-3 py-1 bg-primary text-white text-[10px] font-bold uppercase tracking-widest rounded-full shadow-lg flex items-center gap-1 w-fit'
                      }
                    >
                      {badge === 'Verified' && <span className="material-symbols-outlined text-[12px]">verified</span>}
                      {badge}
                    </span>
                  ))}
                </div>
                ...
                >
                  <button
                    ...
                    className="size-10 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center text-white hover:bg-white hover:text-red-500 transition-all"
                  >
