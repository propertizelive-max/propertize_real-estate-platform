...
export default function Search() {
  ...
  const state = location.state as { results?: PropertyWithMedia[]; fromSearch?: boolean } | null
  const [results, setResults] = useState<PropertyWithMedia[]>(state?.results ?? [])
  const [loading, setLoading] = useState(!q ? false : !state?.results?.length)
  ...
  useEffect(() => {
    ...
    if (state?.results?.length && state?.fromSearch) {
      setLoading(false)
      return
    }
    let cancelled = false
