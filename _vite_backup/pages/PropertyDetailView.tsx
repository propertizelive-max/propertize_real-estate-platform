...
function AvailableConfigurations({ units }: { units: PropertyUnitRow[] }) {
  ...
  const bhkTabs = Object.keys(byBhk)
    .map(Number)
    .filter((n) => n > 0)
    .sort((a, b) => a - b)
  ...
  const minPrice = activeUnits.length
    ? Math.min(...activeUnits.map((u) => u.price ?? Infinity).filter((p) => p !== Infinity))
    : null
  const minSqft = activeUnits.length
    ? Math.min(...activeUnits.map((u) => u.square_feet ?? Infinity).filter((s) => s !== Infinity))
    : null
      ...
      <div className="flex gap-6 border-b border-slate-200 mb-6">
        {bhkTabs.map((bhk) => (
          <button
            ...
            className={`pb-3 font-bold text-sm transition-colors border-b-2 -mb-px ${
              activeBhk === bhk
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-primary">
                {minSqft != null && minSqft !== Infinity
                  ? `${minSqft.toLocaleString()} sq.ft.`
                  : '—'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-primary">
                {baths !== '—' ? `${baths} Bath${Number(baths) !== 1 ? 's' : ''}` : baths}
              </p>
            </div>
          </div>
        </div>
...
export default function PropertyDetailView() {
  ...
  const [similar, setSimilar] = useState<PropertyWithMedia[]>([])
    ...
    async function load() {
      ...
      try {
        ...
        if (prop?.listing_type) {
          const sim = await fetchSimilarProperties(prop.listing_type, propId, 3)
          setSimilar(sim)
        } else {
          setSimilar([])
        }
  ...
  if (loading) {
    return (
      <div className="luxe-landing bg-background-light dark:bg-background-dark font-display min-h-screen flex items-center justify-center">
        <p className="text-primary/60 font-medium">Loading property...</p>
      </div>
    )
  }
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <h2 className="text-4xl font-bold text-primary mb-2">{property.title ?? 'Untitled'}</h2>
          ...
          <div className="col-span-12 lg:col-span-9">
            ...
            <div className="flex gap-4 mt-4 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              <button
                ...
                className={`min-w-[160px] h-24 rounded-lg overflow-hidden flex-shrink-0 border-2 transition-all ${selectedImage === 0 ? 'border-accent-gold shadow-sm' : 'border-transparent hover:border-slate-300'}`}
              >
                <img className="w-full h-full object-cover" alt="Main view" src={mainImage} />
              </button>
              {thumbnails.map((t, i) => (
                <button
                  key={t.id}
                  ...
                  className={`min-w-[160px] h-24 rounded-lg overflow-hidden flex-shrink-0 border-2 transition-all ${selectedImage === i + 1 ? 'border-accent-gold shadow-sm' : 'border-transparent hover:border-slate-300'}`}
                >
                  <img className={`w-full h-full object-cover ${selectedImage !== i + 1 ? 'opacity-80 hover:opacity-100' : ''}`} alt="" src={t.file_url} />
                </button>
              ))}
              {media.length > 5 && (
                <div className="min-w-[160px] h-24 rounded-lg overflow-hidden relative border-2 border-transparent cursor-pointer flex-shrink-0">
                  <img className="w-full h-full object-cover opacity-60" alt="More" src={thumbnails[thumbnails.length - 1]?.file_url ?? mainImage} />
              ...
              <div className="pt-6 border-t border-slate-100">
                <div className="flex items-center gap-4 mb-4">
                  <img
                    ...
                    alt="Agent professional portrait"
                    src="https://lh3.googleusercontent.com/aida-public/AB6AXuBpSLztKIpn1WsCSqYqJ8NtI9s6GPjx79AR8CvVFNfXP0ezL1mEFW5brzLiN5-LBUWsF72l2rq8FaQvm0uJ3G2pj2WsuBs90YAjt0MQQ1ucTiqsqLQ3XThw2fbyr3zxHY8occBGe8hF3oeOEWlvI1JUwsZvnidqrJXe9iBK24BHswRSGset168Hv5ceQL9yvywoQkSwkdPSti9eZN596Ahdreubcu9rYsJXtnyyLaYXd2fMoqoA785_adUDsAg0ZxPMnuFYmu4QnXJ5"
                  />
                  <div>
                ...
                <div className="space-y-3">
                  ...
                  <Link
                    to={`/property/${id ?? '1'}/appointment`}
                    className="w-full border-2 border-primary text-primary font-bold py-3.5 rounded-lg hover:bg-primary hover:text-white transition-all flex items-center justify-center gap-2"
                  >
              ...
              <div className="bg-slate-50 p-4 rounded-lg">
                ...
                <ul className="text-xs text-slate-600 space-y-2">
                  <li className="flex items-center gap-2 font-medium">
                    ...
                    Certified Pre-Inspection
                  </li>
                  <li className="flex items-center gap-2 font-medium">
                    <span className="material-symbols-outlined text-accent-gold text-base">local_shipping</span>
                    VIP Relocation Package
                  </li>
                </ul>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-9 gap-12">
          <div className="lg:col-span-6 space-y-16">
            ...
            {/* Amenities - name only, no description */}
            {property.amenities?.length ? (
            <section>
              <h3 className="text-2xl font-bold text-primary mb-8">Amenities & Features</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                {property.amenities.map((a) => (
                  <div key={a.id} className="flex items-start gap-4">
                    ...
                    <div>
                      <h4 className="font-bold text-primary">{a.name}</h4>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Configurations - only for Projects, grouped by BHK */}
            {property.listing_type === 'Project' && property.units?.length ? (
            <AvailableConfigurations units={property.units} />
            ) : null}
            ...
            {/* Map */}
            <section>
              <h3 className="text-2xl font-bold text-primary mb-6">Location & Neighborhood</h3>
              <div className="rounded-xl overflow-hidden shadow-sm h-96 relative border border-slate-200">
                <img
                  className="w-full h-full object-cover"
                  alt="Malibu, California"
                  src="https://lh3.googleusercontent.com/aida-public/AB6AXuB5mGQ_LWZfp7vBA1ScpZZWqy8qrA4iyDTTowUCpumHVolRg5Mpko1nnvsIspJP0f8v1vdrEMMzx0RdTi3YvASvEtHy2pbJthOKVPIT9zdtq6HGVw7k80XZggxJtT2fQGi15-nOONOJ68OAulYBd_tpMc0kMSGBbApGqE2rxcA2WBOGSOoIHjhULw00OP6LV0VWFzJmV3D2n9hwhLh1MABSIcjnnswmtrIVbmrToHImn8DdrwlV1MJMChDwZ_Bk4MBmaPKq_7Ab-R_e"
                />
        ...
        <section className="mt-24 pt-24 border-t border-slate-200">
          <div className="flex items-center justify-between mb-10 flex-wrap gap-4">
            <h3 className="text-3xl font-bold text-primary">Similar Exclusive Listings</h3>
            <div className="flex gap-2">
              <button type="button" className="p-3 border-2 border-slate-200 rounded-full hover:bg-slate-100 transition-all">
                <span className="material-symbols-outlined">arrow_back</span>
              </button>
              <button type="button" className="p-3 bg-primary text-white rounded-full hover:bg-primary/90 transition-all shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {similar.map((item) => (
              <Link key={item.id} to={`/property/${item.id}`} className="group cursor-pointer block">
                <div className="relative aspect-[4/3] rounded-xl overflow-hidden mb-4 shadow-md transition-all group-hover:shadow-xl group-hover:-translate-y-1">
                  <img className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" alt={item.title ?? ''} src={getFirstImage(item.media)} />
                  <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm p-2 rounded-full text-primary shadow-sm">
                ...
                <div>
                  <h4 className="text-xl font-bold text-primary mb-1">{item.title ?? 'Untitled'}</h4>
      ...
      <footer className="bg-primary text-white mt-24 py-16">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12">
          <div className="col-span-1 md:col-span-2">
            ...
            <div className="flex gap-4">
              <a className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-accent-gold transition-colors" href="#">
          ...
          <div>
            <h5 className="font-bold mb-6 text-accent-gold uppercase tracking-widest text-xs">Navigation</h5>
            <ul className="space-y-4 text-slate-300 text-sm">
              <li><Link to="/projects" className="hover:text-white transition-colors">Find a Property</Link></li>
          ...
          <div>
            <h5 className="font-bold mb-6 text-accent-gold uppercase tracking-widest text-xs">Legal</h5>
            <ul className="space-y-4 text-slate-300 text-sm">
        ...
        <div className="max-w-7xl mx-auto px-6 pt-16 mt-16 border-t border-white/10 text-center">
          <p className="text-slate-500 text-xs">
            © {new Date().getFullYear()} Propertize. All rights reserved. Equal Housing Opportunity.
          </p>
        </div>
      </footer>
    </div>
