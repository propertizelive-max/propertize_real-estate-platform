...
export default function BookAppointment() {
  ...
  const handleBook = async () => {
    ...
    try {
      ...
      setSuccess(true)
      setTimeout(() => navigate(`/property/${propertyId}`), 2000)
    } catch (e: unknown) {
    ...
    <div className="bg-background-light dark:bg-background-dark text-primary min-h-screen font-display">
      <main className="max-w-7xl mx-auto px-6 py-12 lg:py-20">
        <div className="grid lg:grid-cols-12 gap-16 items-start">
          {/* Left: Intro (no Your Details) */}
          <div className="lg:col-span-5 space-y-8">
            <div className="space-y-4">
              <span className="text-primary/60 font-semibold tracking-widest text-xs uppercase">Exclusive Access</span>
              <h2 className="text-5xl font-black leading-[1.1] tracking-tight text-primary">Schedule a Private Viewing</h2>
          ...
          <div className="lg:col-span-7">
            <div className="bg-white dark:bg-white/5 rounded-xl shadow-2xl shadow-primary/5 border border-primary/10 overflow-hidden">
              <div className="p-8 md:p-10 space-y-10">
                ...
                <div className="grid md:grid-cols-2 gap-12">
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-7 gap-1">
                      {prevDays.map((d) => (
                        <div key={`prev-${d}`} className="p-3 text-sm text-primary/20">{d}</div>
                      ))}
                      {currDays.map((d) => {
                  ...
                  <div className="space-y-6">
                    <h4 className="font-bold text-lg">Available Slots</h4>
                ...
                {success && (
                  <div className="p-4 bg-green-50 text-green-700 rounded-lg text-sm" role="alert">
                ...
                <div className="bg-background-light dark:bg-primary/10 p-6 md:p-8 rounded-xl flex flex-col md:flex-row items-center justify-between gap-6 border border-primary/5">
                  ...
                  <button
                    ...
                    onClick={handleBook}
                    disabled={submitting}
                    className="w-full md:w-auto bg-primary text-white px-10 py-5 rounded-lg text-base font-black uppercase tracking-widest hover:bg-primary/90 disabled:opacity-50 transition-all shadow-lg shadow-primary/20 active:scale-[0.98]"
                  >
            ...
            <div className="mt-8 flex items-center gap-6 px-4">
              <img
                ...
                src={getFirstImage(property.media ?? [])}
              />
              <div>
