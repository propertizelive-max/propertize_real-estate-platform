...
type ListingRow = { id: number; title: string | null; price: number | null; cover_url?: string | null }

function KpiIcon({ name }: { name: string }) {
  const base = 'w-10 h-10 rounded-full flex items-center justify-center'
  if (name === 'building') return <div className={`${base} bg-blue-100 text-blue-600`}><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg></div>
  if (name === 'users') return <div className={`${base} bg-blue-100 text-blue-600`}><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg></div>
  if (name === 'calendar') return <div className={`${base} bg-blue-100 text-blue-600`}><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg></div>
    ...
    async function load() {
      ...
      try {
        ...
        if (list.length > 0) {
          ...
          const { data: mediaRows } = await supabase.from(TABLE.property_media).select('property_id, file_url').in('property_id', ids).order('id')
          ...
          setRecentListings(list.map((p) => ({ ...p, cover_url: firstByProp[p.id] ?? null })))
        } else {
          setRecentListings([])
        }
      } catch (e) {
    ...
    <div className="space-y-6">
        ...
        {kpis.map((k) => (
          <div key={k.title} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div className="flex items-start justify-between">
              <KpiIcon name={k.icon} />
              {k.change && <span className={`text-sm font-medium ${k.positive ? 'text-green-600' : 'text-red-600'}`}>{k.change}</span>}
            </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-gray-100">
            ...
            <Link href="/admin/scheduled-tours" className="text-sm text-primary hover:underline">View Calendar</Link>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-gray-100">
            ...
            <Link href="/admin/properties/add" className="text-sm text-primary hover:underline">Add New</Link>
          </div>
          {loading ? (
            ...
            <ul className="divide-y divide-gray-100">
              {recentListings.map((l) => (
                <li key={l.id}>
                  <Link href={`/admin/properties/${l.id}`} className="flex items-center justify-between p-4 hover:bg-gray-50">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-12 h-12 rounded-lg bg-gray-200 shrink-0 overflow-hidden">
                        {l.cover_url ? <img src={l.cover_url} alt="" className="w-full h-full object-cover" /> : null}
                      </div>
                      <div className="min-w-0">
                        <p className="font-medium text-gray-900 truncate">{l.title ?? 'Untitled'}</p>
                        <p className="text-sm text-gray-500">{l.price != null ? l.price.toLocaleString() : '—'}</p>
                      </div>
                    </div>
          ...
          <div className="p-4 border-t border-gray-100">
            <Link href="/admin/properties" className="block w-full py-2 text-center border-2 border-primary text-primary font-medium rounded-lg hover:bg-primary hover:text-white transition-colors">View All Listings</Link>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#1e293b] rounded-xl p-6 text-white relative overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="mt-2 text-gray-300 text-sm max-w-2xl">Property demand insights and marketing suggestions can be added here.</p>
        </div>
      </div>
    </div>
  )
